...
/**
  ...
  * Do not self delete from this function, `return COMPONENT_INCOMPATIBLE` instead
  */
...
/**
  ...
  */
/datum/component/COMPONENT_COMPAT_DATUM_CLEANUP
	parent = null
	return ..()

/datum/component/proc/__cleanup(force, silent)
	if(!force && parent)
		_RemoveFromParent()
	if(!silent)
		SEND_SIGNAL(parent, COMSIG_COMPONENT_REMOVING, src)
	COMPONENT_COMPAT_DELETE(src)
...
/datum/proc/RegisterSignal(datum/target, sig_type_or_types, proctype, override = FALSE)
	if(COMPONENT_COMPAT_DELETED(src) || COMPONENT_COMPAT_DELETED(target))
		return
	...
	var/list/sig_types = istype(sig_type_or_types, /list) ? sig_type_or_types : list(sig_type_or_types)
	for(var/sig_type in sig_types)
		if(!override && procs[target][sig_type])
			COMPONENT_COMPAT_STACK_TRACE("[sig_type] overridden. Use override = TRUE to suppress this warning")
...
/datum/proc/UnregisterSignal(datum/target, sig_type_or_types)
	...
	if(!signal_procs || !signal_procs[target] || !lookup)
		return
	if(!istype(sig_type_or_types, /list))
	...
	for(var/sig in sig_type_or_types)
		switch(length(lookup[sig]))
			...
			if(1)
				COMPONENT_COMPAT_STACK_TRACE("[target] ([target.type]) somehow has single length list inside comp_lookup")
...
/datum/component/proc/_GetInverseTypeList(our_type = type)
	...
	while (current_type != /datum/component)
		current_type = COMPONENT_COMPAT_TYPE_TO_PARENT(current_type)
		. += current_type
...
/datum/proc/_SendSignal(sigtype, list/arguments)
	...
	if(!length(target))
		...
		return NONE | COMPONENT_COMPAT_CALL_ASYNC(C, proctype, arguments)
	. = NONE
	for(var/I in target)
		var/datum/C = I
		if(!C.signal_enabled)
			continue
		var/proctype = C.signal_procs[src][sigtype]
		. |= COMPONENT_COMPAT_CALL_ASYNC(C, proctype, arguments)
...
/datum/proc/GetComponent(datum/component/c_type)
	...
	if(initial(c_type.dupe_mode) == COMPONENT_DUPE_ALLOWED)
		COMPONENT_COMPAT_STACK_TRACE("GetComponent was called to get a component of which multiple copies could be on an object. This can easily break and should be changed. Type: \[[c_type]\]")
...
/datum/proc/AddComponent(new_type, ...)
		...
		if(old_comp)
			switch(dm)
				if(COMPONENT_DUPE_UNIQUE)
					...
					if(!COMPONENT_COMPAT_DELETED(new_comp))
						old_comp.InheritComponent(new_comp, TRUE)
						COMPONENT_COMPAT_DELETE(new_comp)
						new_comp = null
				if(COMPONENT_DUPE_HIGHLANDER)
					...
					if(!COMPONENT_COMPAT_DELETED(new_comp))
						...
						COMPONENT_COMPAT_DELETE(old_comp)
						old_comp = null
	...
	if(!old_comp && !COMPONENT_COMPAT_DELETED(new_comp)) // Nothing related to duplicate components happened and the new component is healthy
...
/datum/proc/TakeComponent(datum/component/target)
	...
	switch(result)
		if(COMPONENT_INCOMPATIBLE)
			...
			COMPONENT_COMPAT_DELETE(target)
